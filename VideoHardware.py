from Components.config import config, ConfigSubDict, ConfigSelection, ConfigNothing, NoSave, ConfigYesNo
...
class VideoHardware:
	...
	hdmi_hw_types = set(["dm500", "dm800se", "dm7020hd", "bm750", "duo", "solo", "uno", "ultimo", "solo2", "duo2", "solose", "zero", "solo4k", "ultimo4k", "uno4k", "uno4kse", "zero4k", "duo4k", "duo4kse"])
	hdmi_pc_hw_types = set(["dm500", "dm800se", "dm7020hd", "bm750", "duo", "solo", "uno", "ultimo", "solo2", "duo2", "solose", "zero", "solo4k", "ultimo4k", "uno4k", "uno4kse", "zero4k", "duo4k", "duo4kse"])
	noscart_hw_types = set(["zero", "solo4k", "ultimo4k", "uno4k", "uno4kse", "zero4k", "duo4k", "duo4kse"])
	noypbpr_hw_types = set(["solose", "zero", "dm8008", "sf8008", "uno4k", "uno4kse", "zero4k", "duo4k", "duo4kse"])
	...
	def isVumodel(self, hw_type):
		return hw_type in set(["bm750", "duo", "solo", "uno", "ultimo", "solo2", "duo2", "solose", "zero", "solo4k", "ultimo4k", "uno4k", "uno4kse", "zero4k", "duo4k", "duo4kse"])

	def isVumodel4K(self, hw_type):
		return hw_type in set(["solo4k", "ultimo4k", "uno4k", "uno4kse", "dm8008", "sf8008", "duo4kse"])
	...
	def getOutputAspect(self):
		...
		if port not in config.av.videomode:
			vtilog("current port is not available. force 16:9")
		else:
	...
	def __init__(self):
		...
		if self.modes.has_key("DVI-PC") and not self.getModeList("DVI-PC"):
			vtilog("remove DVI-PC because it does not exist.")
			del self.modes["DVI-PC"]

		if self.isNoScart(self.getDeviceName()):
			if self.modes.has_key("Scart"):
				vtilog("remove Scart because it does not exist.")
				del self.modes["Scart"]

		if self.isNoYPbPr(self.getDeviceName()):
			if self.modes.has_key("YPbPr"):
				vtilog("remove YPbPr because it does not exist.")
	...
	def readAvailableModes(self):
		try:
			modes = open("/proc/stb/video/videomode_choices").read()
			modes = modes.rstrip()
			self.modes_available = modes.split(' ')
		except IOError:
			vtilog("failed to read video_choices.")
	...
	def readPreferredModes(self):
		try:
			modes = open("/proc/stb/video/videomode_preferred").read()
			modes = modes.rstrip()
			self.modes_preferred = modes.split(' ')
		except IOError:
			vtilog("failed to read video_preferred.")
			self.modes_preferred = self.modes_available

		if self.modes_preferred != self.last_modes_preferred:
			self.last_modes_preferred = self.modes_preferred
			vtilog("hotplug on DVI")
	...
	def createConfig(self):
		...
		for port in port_choices:
			...
			for mode in mode_choices:
				...
				multi24_default = "multi" + " (50/60/24p)"
				has_24p_multi = multi24_default in rate_choices and True or False
				for rate in rate_choices:
					if rate == multi24_default:
						if self.has24pChoices:
							ratelist.append( (rate, rate))
					else:
						ratelist.append( (rate, rate))

				if self.has24pChoices and has_24p_multi:
					config.av.videorate[mode[0]] = ConfigSelection(choices = ratelist, default = multi24_default)
				else:
		...
		if self.has24pChoices:
			config.av.autoframerate_enable = ConfigYesNo(default = False)
			config.av.autoframerate_mode = ConfigSelection(choices = [("manu", _("manually")), ("auto", _("automatically"))], default = "manu")
		else:
			config.av.autoframerate_enable = ConfigYesNo(default = True)
			config.av.autoframerate_mode = ConfigSelection(choices = [("manu", _("manually")), ("auto", _("automatically"))], default = "auto")

		if os.path.exists("/proc/stb/video/hdmi_colorspace"):
			def setHdmiColorspace(config):
				try:
					vtilog("set HDMI Colorspace : ",config.value)
					f = open("/proc/stb/video/hdmi_colorspace", "w")
					f.write(config.value)
					f.close()
				except IOError:
					vtilog("set HDMI Colorspace failed!")
		...
		if os.path.exists("/proc/stb/video/hdmi_colordepth"):
			def setHdmiColordepth(config):
				try:
					vtilog("set HDMI Colordepth : ",config.value)
					f = open("/proc/stb/video/hdmi_colordepth", "w")
					f.write(config.value)
					f.close()
				except IOError:
					vtilog("set HDMI Colordepth failed!")
	...
	def setVideomode(self, port, mode, rate):
		if port is None or port not in config.av.videomode:
			vtilog("current port not available. couldn't set videomode")
			return

		if mode not in config.av.videorate:
			vtilog("current mode not available. couldn't set videomode")
			return
		...
		vtilog("set Videomode", port, mode, rate)
		...
		if (mode_50 != mode_60):
			try:
				...
				vtilog("cannot open /proc/stb/vide/videomode_50hz or videomode_60hz")

			# Too slow moving to Scart/multi in modeSelectionMoved
			#try:
			#	open("/proc/stb/video/videomode_50hz", "w").write(mode_60)
			#except IOError:
			#	vtilog("cannot open /proc/stb/vide/videomode_60Hz")

		else:
			try:
				open("/proc/stb/video/videomode", "w").write(mode_50)
			except IOError:
				vtilog("cannot open /proc/stb/vide/videomode")

		if self.has24pChoices:
			try:
				open("/proc/stb/video/videomode_24hz", "w").write(mode_24)
			except IOError:
				vtilog("cannot open /proc/stb/vide/videomode_24hz")
	...
	def changedAspect(self, configElement):
		...
		port = config.av.videoport.value
		if port not in config.av.videomode:
			vtilog("current port not available. couldn't set aspect")
			return
	...
	def setAspect(self, aspect, policy, policy2, wss):
		vtilog("set aspect, policy, policy2, wss", aspect, policy, policy2, wss)
	...
	def saveVideomode(self, port, mode, rate):
		vtilog("save Videomode", port, mode, rate)
